...
// Göteborg: ONE feed from Västtrafik (GTFS Regional, the detailed one with
// shapes) covering all of Västra Götaland, cut down to the Göteborg region by
// a precomputed allowlist (scope.mjs → data/scope.json). Two modes out of the
// one bundle: the buses navy and Sweden's largest tram network — lines 1–13
// plus the works-period X — family red. No rail: the feed calls every train
// "TÅG" and gives it no line number, so there is nothing to letter.
// Usage: node pipeline/build.mjs [--all | lines...] [--tram all|3,9]
...
// Exact-match repairs for poles a feed names badly. Västtrafik needs none:
// of 27 204 stop names only nine shout, and each is an acronym the city
...
// ONE feed, TWO cfgs.
//
// Västtrafik publishes GTFS Regional for the whole of Västra Götaland — 611
// bus lines from Strömstad to Skövde over 24 000 km². The map is the
// Göteborg region, so pipeline/scope.mjs precomputes the allowlist
// (data/scope.json). Route types are the extended set: 700 bus, 900 tram.
//
// Cut deliberately: the regional trains (100 — the feed names every one of
// them "TÅG" and gives no line number, so unlike Stockholm's numbered
// pendeltåg there is nothing to draw and label), Närtrafiken (1501, dial-a-
// ride with numbers but no fixed run) and the ferries (1000 — Älvsnabben and
// the Styrsö archipelago; the engine has no water graph).
//
// LINE KEYS: Västtrafik numbers each municipality's local network from 1, so
// four different "1" run in the frame — Kungälv, Öckerö, Mölnlycke and
// Kungsbacka. scope.mjs gives those a town prefix in the KEY (Göteborg, the
// main network, keeps bare keys) and the street number is printed through
// LBL: the Rybnik rule, where the county's W never reaches the pavement.
const LBL = new Map();
...
// them run at once — Frihamnen–Majorna and Marklandsgatan–Drottningtorget.
// They need different keys or the engine welds them into one line jumping
// across town; both still print the single letter the stop signs show.
// The suffix is "#2", not "2": a plain one made the key "X2", which is a real
// Västtrafik express BUS, and the bus inherited the tram's printed label.
// No line name in this feed contains a hash.
const tramSeen = new Map();
const tramKey = (sn) => {
  if (!sn) return null;
...
const MODES = [{
  ...
  // Göteborg and its kommuner — 4 × 4 tiles cut out of the Geofabrik extract
  ...
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['16']),
  feeds: [
    { tag: 'vt', dir: 'data/gtfs', routeTypes: ['700'],
      skipRoute: (r) => !S_BUS.has(r.route_id), mapKey: busKey },
  ],
}];
const tramAll = tramLines.length === 1 && tramLines[0] === 'all';
if (tramAll || tramLines.length) MODES.push({
  mode: 'tram', label: 'trams', osmFile: 'data/osm/goteborg-rail.json',
  graphMode: 'tram', railKeep: new Set(['tram', 'light_rail']),
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : tramLines,
  feeds: [
    { tag: 'vt', dir: 'data/gtfs', routeTypes: ['900'],
      skipRoute: (r) => !S_TRAM.has(r.route_id), mapKey: tramKey },
  ],
});
